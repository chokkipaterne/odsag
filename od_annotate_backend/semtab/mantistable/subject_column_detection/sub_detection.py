...
from semtab.mantistable.subject_column_detection.metric import EMCMetric, UCMetric, AWMetric, \
    DFMetric
...
class SubDetection:
    ...
    def get_sub_colRve(self, tableRve, tableDataRve):
        table = tableRve
        score_list = []
        first_ne = 0
        info_table = tableDataRve.infoTableRve
        ...
        for i, necol in enumerate(ne_cols):
            ...
            for metric_key in metric_map:
                ...
                score = metric_map[metric_key].computeRve(tableDataRve)
