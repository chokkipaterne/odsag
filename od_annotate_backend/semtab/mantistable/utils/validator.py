...
from semtab.mantistable.utils.nlp import utils as nlp
from semtab.mantistable.normalization.converter import Converter
from semtab.mantistable.utils.assets.assets import Assets
...
class Validator:
    ...
    def is_numeric(s):
        def is_unit(s):
            if Validator.is_decimal(s):
                unit_list = Converter().get_units_list()
        ...
        return Validator.is_decimal(s) or is_unit(s.lower())
    ...
    def is_currency(s):
        if len(s) < 15:
            ...
            # If it is a single word composed of both digits and letters
            if countws == 0:
                if valuex in Assets().load_list("currency.txt"):
                    return True
        ...
        if len(s) == 3 or (3 < len(words) < 9):
            ...
            if len(words) < 3:
                for i in range(0, len(words)):
                    if 2 < len(words[i]) < 5:
                        if words[i].upper() in Assets().load_list("iatacodes.txt"):
                            return True
